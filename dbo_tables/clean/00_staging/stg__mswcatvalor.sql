with stg_mswcatvalor as (
	select
		--{{ dbt_utils.generate_surrogate_key(['col_pk1', 'col_pk2']) }} as pk_mswcatvalor,
		...
		safe_cast(`nomvaloringles` as STRING) as nomvaloringles,
		...
		safe_cast(`nomvalorotroidioma` as STRING) as nomvalorotroidioma,
		safe_cast(`_ab_cdc_deleted_at` as STRING) as _ab_cdc_deleted_at,
		safe_cast(`_ab_cdc_updated_at` as STRING) as _ab_cdc_updated_at,
		safe_cast(`etiquetadescotroidioma` as STRING) as etiquetadescotroidioma,
		safe_cast(`_ab_cdc_event_serial_no` as STRING) as _ab_cdc_event_serial_no
	from {{source('src_prod_kraken_abt','MSWSch_MswCatValor')}}
)
select * from stg_mswcatvalor