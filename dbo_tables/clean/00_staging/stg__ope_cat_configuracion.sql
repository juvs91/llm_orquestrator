with stg_opecatconfiguracion as (
	select
		--{{ dbt_utils.generate_surrogate_key(['col_pk1', 'col_pk2']) }} as pk_opecatconfiguracion,
	...
	from {{source('src_prod_kraken_abt','OpeSch_OpeCatConfiguracion')}}
)
select * from stg_opecatconfiguracion