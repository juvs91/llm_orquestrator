with stg_acecatarea as (
	select
		--{{ dbt_utils.generate_surrogate_key(['col_pk1', 'col_pk2']) }} as pk_acecatarea,
	...
	from {{source('src_prod_kraken_abt','dbo_AceCatArea')}}
)
select * from stg_acecatarea