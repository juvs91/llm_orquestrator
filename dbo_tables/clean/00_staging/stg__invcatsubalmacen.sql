with stg_invcatsubalmacen as (
	select
		--{{ dbt_utils.generate_surrogate_key(['col_pk1', 'col_pk2']) }} as pk_invcatsubalmacen,
		...
		safe_cast(`nomsubalmacen` as STRING) as nomsubalmacen,
	...
	from {{source('src_prod_kraken_abt','dbo_InvCatSubAlmacen')}}
)
select * from stg_invcatsubalmacen