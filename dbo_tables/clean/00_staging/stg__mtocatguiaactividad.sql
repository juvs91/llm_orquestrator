with stg_mtocatguiaactividad as (
	select
		--{{ dbt_utils.generate_surrogate_key(['col_pk1', 'col_pk2']) }} as pk_mtocatguiaactividad,
		...
		safe_cast(`nomguiaactividad` as STRING) as nomguiaactividad,
	...
	from {{source('src_prod_kraken_abt','MtoSch_MtoCatGuiaActividad')}}
)
select * from stg_mtocatguiaactividad