with stg_vtacatcolonia as (
	select
		--{{ dbt_utils.generate_surrogate_key(['col_pk1', 'col_pk2']) }} as pk_vtacatcolonia,
		...
		safe_cast(`nomcolonia` as STRING) as nomcolonia,
	...
	from {{source('src_prod_kraken_abt','dbo_VtaCatColonia')}}
)
select * from stg_vtacatcolonia