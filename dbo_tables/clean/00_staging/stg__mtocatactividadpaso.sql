with stg_mtocatactividadpaso as (
	select
		--{{ dbt_utils.generate_surrogate_key(['col_pk1', 'col_pk2']) }} as pk_mtocatactividadpaso,
		...
		safe_cast(`claactividadpaso` as INT64) as claactividadpaso,
	...
	from {{source('src_prod_kraken_abt','MtoSch_MtoCatActividadPaso')}}
)
select * from stg_mtocatactividadpaso