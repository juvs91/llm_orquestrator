with stg_mswcatcaracteristica as (
	select
		--{{ dbt_utils.generate_surrogate_key(['col_pk1', 'col_pk2']) }} as pk_mswcatcaracteristica,
	...
	from {{source('src_prod_kraken_abt','MSWSch_MswCatCaracteristica')}}
)
select * from stg_mswcatcaracteristica