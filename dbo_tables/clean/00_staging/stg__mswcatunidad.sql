with stg_mswcatunidad as (
	select
		--{{ dbt_utils.generate_surrogate_key(['col_pk1', 'col_pk2']) }} as pk_mswcatunidad,
		safe_cast(`_airbyte_raw_id` as STRING) as _airbyte_raw_id,
		safe_cast(`_airbyte_extracted_at` as TIMESTAMP) as _airbyte_extracted_at,
		safe_cast(`_airbyte_meta` as JSON) as _airbyte_meta,
		safe_cast(`claunidad` as INT64) as claunidad,
		safe_cast(`nomunidad` as STRING) as nomunidad,
	...
	from {{source('src_prod_kraken_abt','FESCH_MswCatUnidad')}}
)
select * from stg_mswcatunidad