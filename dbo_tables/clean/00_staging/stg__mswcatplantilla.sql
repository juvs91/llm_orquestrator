with stg_mswcatplantilla as (
	select
		--{{ dbt_utils.generate_surrogate_key(['col_pk1', 'col_pk2']) }} as pk_mswcatplantilla,
	...
	from {{source('src_prod_kraken_abt','MSWSch_MswCatPlantilla')}}
)
select * from stg_mswcatplantilla