with stg_mtocatsubarea as (
	select
		--{{ dbt_utils.generate_surrogate_key(['col_pk1', 'col_pk2']) }} as pk_mtocatsubarea,
		...
		safe_cast(`nomsubarea` as STRING) as nomsubarea,
		safe_cast(`nombrepcmod` as STRING) as nombrepcmod,
		safe_cast(`_ab_cdc_lsn` as STRING) as _ab_cdc_lsn,
		safe_cast(`claubicacion` as INT64) as claubicacion,
		safe_cast(`clavesubarea` as STRING) as clavesubarea,
	...
	from {{source('src_prod_kraken_abt','MtoSch_MtoCatSubArea')}}
)
select * from stg_mtocatsubarea