with stg_opecatseccion as (
	select
		--{{ dbt_utils.generate_surrogate_key(['col_pk1', 'col_pk2']) }} as pk_opecatseccion,
		safe_cast(`_airbyte_raw_id` as STRING) as _airbyte_raw_id,
		safe_cast(`_airbyte_extracted_at` as TIMESTAMP) as _airbyte_extracted_at,
		safe_cast(`_airbyte_meta` as JSON) as _airbyte_meta,
		safe_cast(`bajalogica` as INT64) as bajalogica,
		safe_cast(`claalmacen` as INT64) as claalmacen,
		safe_cast(`claseccion` as INT64) as claseccion,
		safe_cast(`nomseccion` as STRING) as nomseccion,
	...
	from {{source('src_prod_kraken_abt','dbo_OpeCatSeccion')}}
)
select * from stg_opecatseccion