with stg_nomcatarea as (
	select
		--{{ dbt_utils.generate_surrogate_key(['col_pk1', 'col_pk2']) }} as pk_nomcatarea,
		...
		safe_cast(`usuario` as INT64) as usuario,
	...
	from {{source('src_prod_kraken_abt','dbo_NomCatArea')}}
)
select * from stg_nomcatarea