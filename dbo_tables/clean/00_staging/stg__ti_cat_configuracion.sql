with stg_ticatconfiguracion as (
	select
		--{{ dbt_utils.generate_surrogate_key(['col_pk1', 'col_pk2']) }} as pk_ticatconfiguracion,
	...
	from {{source('src_prod_kraken_abt','dbo_TiCatConfiguracion')}}
)
select * from stg_ticatconfiguracion